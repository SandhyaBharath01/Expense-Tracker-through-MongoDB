...
expensesPerPageSelect.addEventListener("change", getAllExpenses);
...
async function getAllExpenses() {
  ...
  try {
    ...
    const token = localStorage.getItem("token");
    const expensesPerPage = expensesPerPageSelect.value;
    const res = await axios.get(
      `http://localhost:2222/expense/getAllExpenses/${currentPage}?expensesPerPage=${expensesPerPage}`,
      { headers: { Authorization: token } }
    );
...
function updatePaginationUI() {
  ...
  if (totalPages > 1) {
    const prevPageLi = createPaginationListItem(currentPage - 1, "Previous", currentPage > 1);
    paginationUL.appendChild(prevPageLi);

      for (let i = 1; i <= totalPages; i++) {
  const pageLi = createPaginationListItem(i, i, i !== currentPage);
  if (i === currentPage) {
  ...
  paginationUL.appendChild(pageLi);
    ...
    const nextPageLi = createPaginationListItem(currentPage + 1, "Next", currentPage < totalPages);
    paginationUL.appendChild(nextPageLi);
  }


function createPaginationListItem(pageNumber, text, isEnabled) {
